...
import type { TableName, TableRow, FilterFunction, SearchConfig, SortConfig } from "@/shared/types/hooks"
 ...
 */
export function useClientInfiniteQuery<T extends TableName, TResult = TableRow<T>>(
  ...
  return useInfiniteQuery<InfiniteHookResult<TResult>, PostgrestError, InfiniteHookResult<TResult>, QueryKey, number>({
    queryKey: key,
    queryFn: async ({ pageParam }): Promise<InfiniteHookResult<TResult>> => {
      ...
      }) as unknown as PostgrestTransformBuilder<Database["public"], TableRow<T>, TResult[]>

      // Apply filters
      if (options?.filters) {
        // We need to cast selectQuery to FilterBuilder to apply filters
        const filterableQuery = selectQuery as unknown as PostgrestFilterBuilder<
          Database["public"],
          TableRow<T>,
          TResult
        >
        selectQuery = options.filters(filterableQuery) as unknown as PostgrestTransformBuilder<
          Database["public"],
          TableRow<T>,
          TResult[]
        >
      ...
      if (options?.search && options.search.query) {
        const filterableQuery = selectQuery as unknown as PostgrestFilterBuilder<
          ...
          TResult
        >
        const { column, query: searchQuery, type = "ilike", ftsOptions } = options.search

        // Fixed search functionality with specific type handling
        if (type === "fts") {
          const result = filterableQuery.textSearch(String(column), searchQuery, ftsOptions || {})
          selectQuery = result as unknown as PostgrestTransformBuilder<Database["public"], TableRow<T>, TResult[]>
        } else if (type === "ilike" || type === "like") {
          const result = filterableQuery[type](String(column), `%${searchQuery}%`)
          selectQuery = result as unknown as PostgrestTransformBuilder<Database["public"], TableRow<T>, TResult[]>
        } else if (type === "eq") {
