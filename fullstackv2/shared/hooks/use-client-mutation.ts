...
> = {
  ...
  mutationOptions?: Omit<
    UseMutationOptions<TData, TError, MutationVariables<T, TAction>, MutationContext<T>>,
    "mutationFn" | "onMutate"
  >
}
      ...
      try {
        switch (action) {
          ...
          case "update":
            ...
            response = await supabase.from(table).update(data).match(match).select()
            break
          case "delete":
            ...
            break
          case "upsert":
            const conflictConstraint = Array.isArray(primaryKey) ? primaryKey.join(",") : String(primaryKey)
