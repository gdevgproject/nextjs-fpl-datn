...
`,
      filters: (query) => {
        ...
        return query.is("deleted_at", null).order("created_at", { ascending: false })
      },
    ...
    return (
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Sản phẩm mới</h2>
          <Link href="/san-pham?sortBy=newest" className="text-sm font-medium hover:underline self-end sm:self-auto">
            Xem tất cả
          </Link>
        </div>
    ...
    return (
      <section className="space-y-6">
        ...
        <div className="p-8 text-center rounded-lg bg-muted/30">
          <p className="text-muted-foreground mb-4">Không thể tải sản phẩm mới</p>
  ...
  const products = (data.data as any[]).map((product) => ({
    ...
    image_url:
      product.product_images?.find((img: any) => img.is_main)?.image_url || product.product_images?.[0]?.image_url,
