...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const transformedCartItems: CartItem[] = isAuthenticated
    ? (cartItems || []).map((item: any) => ({
        ...
        product_image: item.product_variants.products.product_images.find(
          (img: any) => img.is_main
        )?.image_url,
    ...
    async (variantId: number, quantity: number) => {
            ...
            const enhancedCart = getLocalCart().map((item) => {
              if (item.variant_id === variantId) {
                return {
                  ...
                  product_image: variantData.products.product_images.find(
                    (img: any) => img.is_main
                  )?.image_url,
      ...
      try {
        ...
        if (existingError && existingError.code !== "PGRST116") {
          throw new Error(
            `Failed to check existing item: ${existingError.message}`
          );
        }
  ...
  const updateItem = useCallback(
    async (cartItemId: number, quantity: number) => {
      ...
      if (!isAuthenticated) {
        ...
        if (
          itemToUpdate?.stock_quantity &&
          quantity > itemToUpdate.stock_quantity
        ) {
          toast.error(
            `Chỉ còn ${itemToUpdate.stock_quantity} sản phẩm trong kho`
          );
          return;
        }
    ...
    const syncLocalCartToServer = async () => {
      if (isAuthenticated && !isLoading && cartData?.id && !isMergingCart) {
        ...
        try {
          ...
          const { data: serverCartItems, error: serverCartError } =
            await supabase
              .from("cart_items")
          ...
          if (serverCartError && serverCartError.code !== "PGRST116") {
            console.error(
              `Failed to fetch server cart items: ${serverCartError.message}`
            );
          }
