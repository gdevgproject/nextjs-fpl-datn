  ...
  return (
    <div className="space-y-4 sm:space-y-6 md:space-y-8">
      ...
      <div>
        ...
        {product.release_year && <p className="text-sm text-muted-foreground mt-1">{product.release_year}</p>}
      </div>
      ...
      {/* Short Description */}
      {product.short_description && <p className="text-base text-muted-foreground">{product.short_description}</p>}

      {/* Variant Selector */}
      <div>
        <h3 className="text-sm font-medium mb-2">Dung tích</h3>
        <VariantSelector variants={variants} selectedVariant={selectedVariant} onSelectVariant={setSelectedVariant} />
      </div>
      ...
      <div className="flex items-baseline gap-2">
        {isOnSale ? (
          <>
            ...
            <Badge className="ml-2 bg-red-500">{discountPercentage}% giảm</Badge>
          </>
        ) : (
          <span className="text-2xl font-bold">{selectedVariant ? formatCurrency(selectedVariant.price) : ""}</span>
        )}
      </div>
      ...
      {/* SKU */}
      {selectedVariant && <p className="text-xs text-muted-foreground">SKU: {selectedVariant.sku}</p>}
    </div>
  )
}
