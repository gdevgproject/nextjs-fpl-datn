...
export function UserTable({ search, roleFilter }: UserTableProps) {
  ...
  if (isError) {
    return <div className="text-center py-4 text-red-500">Đã xảy ra lỗi khi tải dữ liệu người dùng</div>
  }
  ...
  return (
    <div className="space-y-4">
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">{user.display_name || "Chưa cập nhật"}</p>
                      </div>
                    </div>
                  </TableCell>
      ...
      {/* Dialogs */}
      {editUser && <UserDialog open={!!editUser} onOpenChange={(open) => !open && setEditUser(null)} user={editUser} />}

      {viewUser && (
        <UserDetailsDialog open={!!viewUser} onOpenChange={(open) => !open && setViewUser(null)} user={viewUser} />
      )}
