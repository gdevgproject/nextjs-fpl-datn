...
import { useCreateProduct, useUpdateProduct, useUploadProductImage } from "../queries"
...
export function ProductForm({ initialData }: ProductFormProps = {}) {
  ...
  const { data: productData, isLoading: isLoadingProduct } = useGetProductById(productId, { enabled: isEditMode })
      ...
      if (uploadedImages.length > 0) {
        ...
        try {
          for (let i = 0; i < uploadedImages.length; i++) {
            ...
            if (i === 0 && (!productData?.images || productData.images.length === 0)) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <Tabs defaultValue="basic" className="w-full">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Mã sản phẩm dùng để phân biệt trong hệ thống</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Nhập mô tả ngắn gọn về sản phẩm" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Nhập mô tả chi tiết về sản phẩm" className="min-h-32" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                          ...
                          <SelectContent>
                            ...
                            {lookups?.concentrations.map((concentration) => (
                              <SelectItem key={concentration.id} value={concentration.id.toString()}>
                                {concentration.name} ({concentration.description})
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
              ...
              <CardContent>
                <div className="space-y-4">
                  {lookups?.productLabels.map((label) => {
                    const labelIndex = form.watch("labels").findIndex((l) => l.label_id === label.id)
                    const isSelected = labelIndex !== -1
                    const validUntil = isSelected ? form.watch(`labels.${labelIndex}.valid_until`) : null
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-col space-y-2">
                              <div className="flex items-center space-x-2">
                                  ...
                                  <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                      ...
                                      if (checked) {
                                        ...
                                        const index = currentLabels.findIndex((l) => l.label_id === label.id)
                                        if (index !== -1) {
                                ...
                                <FormLabel className="font-medium cursor-pointer">{label.name}</FormLabel>
                                <Badge
                                  style={{
                              ...
                              {isSelected && (
                                <div className="pl-6 mt-2">
                                  <FormLabel className="text-sm">Hiệu lực đến (tùy chọn)</FormLabel>
                                  <Controller
                                    control={form.control}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Nhập quốc gia xuất xứ" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Nhập phong cách nước hoa (vd: Hiện đại, Cổ điển...)" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Nhập độ tỏa hương (vd: Nhẹ, Vừa, Mạnh...)" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Nhập độ lưu hương (vd: 2-4 giờ, 4-6 giờ...)" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <TabsContent value="images" className="space-y-6">
            <Card>
              ...
              <CardContent>
                {isEditMode && productData?.images && productData.images.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-sm font-medium mb-2">Hình ảnh hiện tại</h3>
                    <div className="grid grid-cols-4 gap-4">
                      {productData.images.map((image) => (
                        <div key={image.id} className="relative border rounded-md p-1">
                          <img
                            src={image.image_url}
                            alt={image.alt_text || "Product image"}
                            className="w-full h-32 object-contain"
                          />
                          {image.is_main && <Badge className="absolute top-2 right-2">Hình chính</Badge>}
                        </div>
                      ))}
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  {uploadPreviews.length > 0 && (
                    <div>
                      <h3 className="text-sm font-medium mb-2">Hình ảnh sẽ được tải lên ({uploadPreviews.length})</h3>
                      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="flex justify-end space-x-4">
          <Button type="button" variant="outline" onClick={() => router.push("/admin/san-pham")}>
            Hủy
          </Button>
