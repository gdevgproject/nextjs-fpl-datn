...
const productKeys = {
  ...
  inventoryHistory: (variantId: number) => [...productKeys.all, "inventory", variantId] as const,
}
...
export function useUploadProductImages() {
  ...
  return useMutation({
    mutationFn: ({ productId, files }: { productId: number; files: File[] }) => uploadProductImages(productId, files),
    onSuccess: (result, variables) => {
      toast.success(result.message)
      queryClient.invalidateQueries({
