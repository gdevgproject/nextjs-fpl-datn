...
export function InventoryDetailsDialog({ open, onOpenChange, inventory }: InventoryDetailsDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>Thông tin chi tiết về lần thay đổi kho này</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm">{format(new Date(inventory.timestamp), "dd/MM/yyyy HH:mm:ss", { locale: vi })}</p>
            </div>
            <div>
              <h4 className="text-sm font-medium text-muted-foreground">Số lượng thay đổi</h4>
              <Badge
                variant={
                  inventory.change_amount > 0 ? "success" : inventory.change_amount < 0 ? "destructive" : "outline"
                }
                className="mt-1"
              >
          ...
          <div>
            ...
            <p className="text-sm font-medium">{inventory.product_variants?.products?.name}</p>
            <p className="text-sm text-muted-foreground">
              {inventory.product_variants?.volume_ml}ml - SKU: {inventory.product_variants?.sku}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h4 className="text-sm font-medium text-muted-foreground">Tồn kho sau thay đổi</h4>
              <p className="text-sm">{inventory.stock_after_change}</p>
            </div>
            <div>
              <h4 className="text-sm font-medium text-muted-foreground">Người thực hiện</h4>
              <p className="text-sm">
                {updatedByUser ? updatedByUser.display_name : inventory.updated_by ? "Đang tải..." : "Hệ thống"}
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              {orderDetails ? (
                <div className="text-sm space-y-1">
                  ...
                  <p>Ngày đặt: {format(new Date(orderDetails.order_date), "dd/MM/yyyy", { locale: vi })}</p>
