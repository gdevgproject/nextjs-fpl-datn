...
export function CategoryImageUploader({ initialImageUrl, categoryId, onChange }: CategoryImageUploaderProps) {
  const [previewUrl, setPreviewUrl] = useState<string | null>(initialImageUrl || null)
  const [isUploading, setIsUploading] = useState(false)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [previousImageUrl, setPreviousImageUrl] = useState<string | null>(initialImageUrl || null)
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) {
      toast.error("Định dạng file không hợp lệ. Vui lòng chọn file PNG, JPG, GIF hoặc WEBP.")
      return
    }
  ...
  const uploadImage = async (file: File, id: number) => {
    try {
      ...
      toast.error(`Lỗi khi tải lên ảnh: ${error instanceof Error ? error.message : "Unknown error"}`)
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      toast.error(`Lỗi khi xóa ảnh: ${error instanceof Error ? error.message : "Unknown error"}`)
    } finally {
      setIsUploading(false)
    }
  }
