...
import { ChevronDown, ChevronUp, MoreHorizontal, Pencil, Trash2, ArrowUpDown, CheckCircle, XCircle } from "lucide-react"
...
export function BannerTable({
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(`Lỗi khi xóa banner: ${error instanceof Error ? error.message : "Unknown error"}`)
    }
  }
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="cursor-pointer" onClick={() => onSortChange("title")}>
                <div className="flex items-center">
                  Tiêu đề
                  {renderSortIndicator("title")}
                </div>
              </TableHead>
              <TableHead className="cursor-pointer" onClick={() => onSortChange("is_active")}>
                <div className="flex items-center">
                  Trạng thái
                  {renderSortIndicator("is_active")}
                </div>
              </TableHead>
              <TableHead className="cursor-pointer" onClick={() => onSortChange("display_order")}>
                <div className="flex items-center">
                  Thứ tự hiển thị
                  {renderSortIndicator("display_order")}
                </div>
              </TableHead>
              <TableHead className="cursor-pointer" onClick={() => onSortChange("start_date")}>
                <div className="flex items-center">
                  Ngày bắt đầu
                  {renderSortIndicator("start_date")}
                </div>
              </TableHead>
              <TableHead className="cursor-pointer" onClick={() => onSortChange("end_date")}>
                <div className="flex items-center">
                  Ngày kết thúc
            ...
            {banners.map((banner) => (
              <TableRow key={banner.id}>
                ...
                <TableCell>
                  ...
                  {banner.link_url && (
                    <div className="mt-1 text-xs text-blue-500 truncate max-w-[200px]">{banner.link_url}</div>
                  )}
                </TableCell>
                <TableCell>
                ...
                <TableCell>{banner.start_date ? formatDate(banner.start_date) : "Không giới hạn"}</TableCell>
                <TableCell>{banner.end_date ? formatDate(banner.end_date) : "Không giới hạn"}</TableCell>
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem className="text-red-600" onClick={() => handleDeleteClick(banner)}>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn xóa banner "{bannerToDelete?.title}"? Hành động này không thể hoàn tác.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
