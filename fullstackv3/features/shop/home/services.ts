...
export async function fetchFeaturedProducts(limit = 8): Promise<Product[]> {
  ...
  try {
    ...
    if (labelError) {
      ...
      return await fetchNewArrivals(limit); // Fallback to new arrivals
    }
    ...
    if (assignmentsCheckError) {
      console.error(
        "Error checking product_label_assignments:",
    ...
    if (variantsError) {
      console.error(
        "Error fetching variants for featured products:",
        variantsError
      );
    }
 ...
 */
export async function fetchProductsOnSale(limit = 8): Promise<Product[]> {
  const supabase = await getSupabaseServerClient();
  ...
  try {
    ...
    const { data: productsWithSalePrice, error: salePriceError } =
      await supabase
        .from("product_variants")
        .select(
          `
    ...
    if (variantsError) {
      console.error(
        "Error fetching variants for sale products:",
        variantsError
      );
    }
    ...
    console.error("Error in fetchProductsOnSale:", error);
    return await fetchNewArrivals(limit); // Fallback to new arrivals
  }
}
...
export async function fetchBestSellingProducts(limit = 8): Promise<Product[]> {
  ...
  try {
    ...
    if (functionError) {
      ...
      return await fetchFallbackBestSellingProducts(limit); // Fallback method
    }
    ...
    console.error("Error in fetchBestSellingProducts:", error);
    return await fetchFallbackBestSellingProducts(limit); // Fallback method
  }
}
 ...
 */
async function fetchFallbackBestSellingProducts(limit = 8): Promise<Product[]> {
  const supabase = await getSupabaseServerClient();
  ...
  try {
    ...
    if (orderItemsError) {
      console.error(
        "Error fetching order items for best selling products:",
        orderItemsError
      );
      return await fetchNewArrivals(limit); // Fallback to new arrivals
    }
    ...
    if (sortedProductIds.length === 0) {
      console.log(
        "No products found after sorting, falling back to new arrivals"
      );
      return await fetchNewArrivals(limit); // Fallback to new arrivals
    }
    ...
    console.error("Error in fetchFallbackBestSellingProducts:", error);
    return await fetchNewArrivals(limit); // Fallback to new arrivals
  }
}
 ...
 */
export async function fetchFeaturedCategories(limit = 6): Promise<Category[]> {
  const supabase = await getSupabaseServerClient();
 ...
 */
export async function fetchFeaturedBrands(limit = 8): Promise<Brand[]> {
  const supabase = await getSupabaseServerClient();
  ...
  try {
    ...
    console.error("Error in fetchFeaturedBrands:", error);
    return [];
  }
}
