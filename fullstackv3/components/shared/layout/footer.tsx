...
import { Facebook, Instagram, Mail, Phone, MapPin, MessageSquare, Youtube, TwitterIcon as TikTok } from "lucide-react"
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-bold text-xl">{isLoading ? <Skeleton className="h-6 w-24" /> : shopName}</span>
            </div>
            ...
            <div className="mt-4 flex space-x-3">
              {facebookUrl && (
                <Link href={facebookUrl} className="text-muted-foreground hover:text-primary">
                  <Facebook className="h-5 w-5" />
                  <span className="sr-only">Facebook</span>
                </Link>
              )}
              {instagramUrl && (
                <Link href={instagramUrl} className="text-muted-foreground hover:text-primary">
                  <Instagram className="h-5 w-5" />
                  <span className="sr-only">Instagram</span>
                </Link>
              )}
              {tiktokUrl && (
                <Link href={tiktokUrl} className="text-muted-foreground hover:text-primary">
                  <TikTok className="h-5 w-5" />
                  <span className="sr-only">TikTok</span>
                </Link>
              )}
              {youtubeUrl && (
                <Link href={youtubeUrl} className="text-muted-foreground hover:text-primary">
                  <Youtube className="h-5 w-5" />
                  <span className="sr-only">Youtube</span>
                </Link>
              )}
              {messengerUrl && (
                <Link href={messengerUrl} className="text-muted-foreground hover:text-primary">
                  <MessageSquare className="h-5 w-5" />
                  <span className="sr-only">Messenger</span>
                </Link>
              )}
              {zaloUrl && (
                <Link href={zaloUrl} className="text-muted-foreground hover:text-primary">
          ...
          <div>
            ...
            <ul className="mt-2 space-y-2 text-sm">
              <li className="flex items-start gap-2 text-muted-foreground">
                ...
                <span>{isLoading ? <Skeleton className="h-4 w-full" /> : address}</span>
              </li>
              <li className="flex items-center gap-2 text-muted-foreground">
                <Mail className="h-4 w-4 flex-shrink-0" />
                <span>{isLoading ? <Skeleton className="h-4 w-32" /> : email}</span>
              </li>
              <li className="flex items-center gap-2 text-muted-foreground">
                <Phone className="h-4 w-4 flex-shrink-0" />
                <span>{isLoading ? <Skeleton className="h-4 w-24" /> : phone}</span>
              </li>
            </ul>
          </div>
        </div>
