...
async function getOrderData(orderId: number) {
  ...
  const { getOrderDetails, getOrderStatuses, getOrderActivityLog } = await import(
    "@/features/admin/order-management/actions"
  )
  ...
  try {
    ...
    const [orderDetailsResult, orderStatusesResult, activityLogResult] = await Promise.all([
...
export default async function OrderDetailPage({
  ...
  const { orderDetails, orderStatuses, activityLog, error, isAdmin } = await getOrderData(orderId)
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          <AlertDescription>{error || "Không thể tải thông tin đơn hàng. Vui lòng thử lại sau."}</AlertDescription>
        </Alert>
      </div>
    )
  }
