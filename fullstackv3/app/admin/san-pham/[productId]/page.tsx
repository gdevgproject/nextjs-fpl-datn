...
import { getProductById, softDeleteProduct, restoreProduct } from "@/features/admin/product-management/actions"
    ...
    <div className="container max-w-7xl mx-auto pb-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {isDeleted && <span className="px-2 py-1 text-xs bg-destructive/15 text-destructive rounded-md">Đã xóa</span>}
        </div>

        <div className="flex items-center gap-2">
          <form action={isDeleted ? restoreProduct.bind(null, productId) : softDeleteProduct.bind(null, productId)}>
            <Button
              variant="outline"
              type="submit"
              className={isDeleted ? undefined : "text-destructive border-destructive/30 hover:bg-destructive/10"}
            >
              {isDeleted ? (
                <>
      ...
      <div className="p-2 bg-muted/50 rounded-md mb-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <span className="ml-2 font-medium">{product.brand?.name || "Không có"}</span>
          </div>
          <div>
            <span className="text-sm text-muted-foreground">Nồng độ:</span>
            <span className="ml-2 font-medium">{product.concentration?.name || "Không có"}</span>
          </div>
        </div>
      </div>
