...
export async function generateMetadata({ params }: ProductPageProps, parent: ResolvingMetadata): Promise<Metadata> {
  ...
  return {
    ...
    description: product.short_description || `${product.name} - Nước hoa chính hãng tại MyBeauty`,
    openGraph: {
      title: `${product.name} - MyBeauty`,
      description: product.short_description || `${product.name} - Nước hoa chính hãng tại MyBeauty`,
...
export default async function ProductPage({ params, searchParams }: ProductPageProps) {
  ...
  return <ProductDetailPage product={product} showReviewForm={showReviewForm} />
}
