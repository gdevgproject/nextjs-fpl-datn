...
async function getOrderServerSide(orderId: string | null, token: string | null) {
      ...
      {error || !order ? (
        <Card className="max-w-3xl mx-auto">
          <CardHeader className="text-center">
            ...
            <CardDescription>{error || "Không thể tải thông tin đơn hàng"}</CardDescription>
          </CardHeader>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="mb-8 text-center">
            ...
            <p className="text-lg text-muted-foreground">Cảm ơn bạn đã mua hàng tại MyBeauty</p>
          </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <CardTitle className="text-xl">Đơn hàng #{order.order_number}</CardTitle>
                  <CardDescription>Ngày đặt: {formatDate(order.created_at)}</CardDescription>
                </div>
                <div className="px-4 py-2 bg-primary/10 text-primary rounded-md font-medium">{order.status}</div>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <ul className="space-y-1 text-sm">
                    <li>
                      <span className="text-muted-foreground">Tên:</span> {order.customer_name}
                    </li>
                    <li>
                      <span className="text-muted-foreground">Email:</span> {order.customer_email}
                    </li>
                    <li>
                      <span className="text-muted-foreground">Số điện thoại:</span> {order.customer_phone}
                    </li>
                  </ul>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Trạng thái: {order.payment_status}</p>
                  ...
                  {order.payment_method.includes("chuyển khoản") && (
                    <div className="mt-3 p-3 bg-muted rounded-md text-sm">
                      ...
                      <div className="mt-1">
                        <p>
                          Ngân hàng:{" "}
                          <span className="font-medium">BIDV - Ngân hàng TMCP Đầu tư và Phát triển Việt Nam</span>
                        </p>
                        <p>
                        ...
                        <p>
                          Nội dung: <span className="font-medium">Thanh toan #{order.order_number}</span>
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div>
                  <h3 className="font-medium text-lg mb-2">Phương thức vận chuyển</h3>
