...
export async function generateMetadata({ params }: ProductPageProps, parent: ResolvingMetadata): Promise<Metadata> {
  ...
  return {
    ...
    description:
      product.short_description || product.description?.substring(0, 160) || "Khám phá nước hoa cao cấp tại MyBeauty",
    openGraph: {
      images: product.product_images.find((img) => img.is_main)?.image_url
        ? [product.product_images.find((img) => img.is_main)?.image_url as string]
        : [],
    },
  }
}
...
async function fetchProductBySlug(slug: string) {
  ...
  const reviewsWithProfiles = fetchedReviewsRaw.map((review) => {
    const reviewerProfile = review.user_id ? profilesMap.get(review.user_id) : null
    const repliesWithProfiles =
      review.review_replies && Array.isArray(review.review_replies)
        ? review.review_replies.map((reply) => {
            const replierProfile = reply.user_id ? profilesMap.get(reply.user_id) : null
            return {
              ...reply,
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const notePyramid = {
    top: product.product_ingredients.filter((item) => item.scent_type === "top").map((item) => item.ingredients),
    middle: product.product_ingredients.filter((item) => item.scent_type === "middle").map((item) => item.ingredients),
    base: product.product_ingredients.filter((item) => item.scent_type === "base").map((item) => item.ingredients),
  }
