...
export function ProductLabelDialog({ open, onOpenChange, mode, productLabel }: ProductLabelDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{mode === "create" ? "Thêm nhãn sản phẩm mới" : "Chỉnh sửa nhãn sản phẩm"}</DialogTitle>
          <DialogDescription>
            {mode === "create" ? "Thêm một nhãn sản phẩm mới vào hệ thống." : "Chỉnh sửa thông tin nhãn sản phẩm."}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Tên nhãn sản phẩm phải là duy nhất trong hệ thống.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isProcessing}>
                {isProcessing ? "Đang xử lý..." : mode === "create" ? "Tạo nhãn sản phẩm" : "Cập nhật nhãn sản phẩm"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
