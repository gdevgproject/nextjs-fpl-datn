...
interface UseStorageUploadOptions {
  ...
  mutationOptions?: Omit<
    UseMutationOptions<UploadMutationResult, StorageError, UploadMutationVariables, unknown>,
    "mutationFn"
  >
}
 ...
 */
export function useStorageUpload(bucket: StorageBuckets, options?: UseStorageUploadOptions) {
  const queryClient = useQueryClient()
  const { invalidateQueryKeys = [], mutationOptions = {} } = options || {}

  return useMutation<UploadMutationResult, StorageError, UploadMutationVariables>({
    mutationFn: async ({ file, fileOptions, path: explicitPath, createPathOptions }) => {
      let filePath: string
      if (explicitPath) {
      ...
      if (!urlData?.publicUrl)
        throw new StorageError(`Could not retrieve public URL after upload for path: ${filePath}`)
...
interface UseStorageDeleteOptions {
  invalidateQueryKeys?: QueryKey[]
  mutationOptions?: Omit<
    UseMutationOptions<DeleteMutationResult, StorageError, DeleteMutationVariables, unknown>,
    "mutationFn"
  >
}
 ...
 */
export function useStorageDelete(bucket: StorageBuckets, options?: UseStorageDeleteOptions) {
  const queryClient = useQueryClient()
  const { invalidateQueryKeys = [], mutationOptions = {} } = options || {}

  return useMutation<DeleteMutationResult, StorageError, DeleteMutationVariables>({
    mutationFn: async (pathsToDelete) => {
      const pathsArray = Array.isArray(pathsToDelete) ? pathsToDelete : [pathsToDelete]
      if (pathsArray.length === 0) return { data: [] }
      const normalizedPaths = pathsArray.map((p) => p.replace(/\/+/g, "/").replace(/^\/|\/$/g, ""))

      const { data, error } = await supabase.storage.from(bucket).remove(normalizedPaths)
...
interface UseStorageListOptions {
  queryOptions?: Omit<UseQueryOptions<FileObject[], StorageError>, "queryKey" | "queryFn">
 ...
 */
export function useStorageList(bucket: StorageBuckets, path = "", options?: UseStorageListOptions) {
  const normalizedPath = path.replace(/\/+/g, "/").replace(/^\/|\/$/g, "")
  // Include listOptions in the query key for proper caching based on parameters
  const queryKey: QueryKey = ["storage", bucket, "list", normalizedPath || "/", options?.listOptions || {}]
