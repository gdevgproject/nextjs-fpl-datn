...
interface BatchMutationOptions<
  ...
  TItem = unknown,
  TVariables extends BatchMutationVariables<T, TItem> = BatchMutationVariables<T, TItem>,
> {
  /** NOTE: Default optimistic updates NOT provided for batch. Implement custom logic if needed. */
  onOptimisticUpdate?: (queryKey: QueryKey, variables: TVariables, queryClient: QueryClient) => void | Promise<void>
  invalidateQueries?: QueryKey[]
  /** Additional options for the underlying `useMutation` hook. */
  mutationOptions?: Omit<UseMutationOptions<TData, TError, TVariables, unknown>, "mutationFn" | "onMutate">
}
...
export function useClientBatchMutation<
  ...
  TVariables extends BatchMutationVariables<T, TItem> = BatchMutationVariables<T, TItem>,
>(table: T, options?: BatchMutationOptions<T, TData, TError, TItem, TVariables>) {
  ...
  return useMutation<TData, TError, TVariables, unknown>({
    mutationFn: async (variables: TVariables): Promise<TData> => {
      ...
      if (!Array.isArray(items) || items.length === 0) {
        console.warn(`Batch mutation (${action} on ${table}) called with empty items.`)
      ...
      const pkArray = Array.isArray(matchColumn) ? matchColumn.map((key) => String(key)) : [String(matchColumn)]
      ...
      try {
        switch (action) {
          ...
          case "delete":
            if (pkArray.length > 1) {
              throw new Error(`Batch delete with composite keys (${onConflictConstraint}) not directly supported.`)
            }
            const key = pkArray[0]
            const valuesToDelete = items.map((item) => {
              ...
              if (!(key in typedItem) || typedItem[key] === undefined || typedItem[key] === null) {
                throw new Error(`Item missing primary key '${key}' for batch delete.`)
              }
        ...
        console.error(`Supabase batch ${action} error on table ${table}:`, error)
    ...
    onMutate: async (variables: TVariables) => {
      if (options?.onOptimisticUpdate) {
        ...
        await options.onOptimisticUpdate(tableListQueryKey, variables, queryClient)
      }
    ...
    onError: (error: TError, variables: TVariables, context: unknown) => {
      console.error(`Batch mutation error (${variables.action} on ${table}):`, error)
    ...
    onSuccess: (data: TData, variables: TVariables, context: unknown) => {
      const queriesToInvalidate = options?.invalidateQueries ?? [tableListQueryKey]
      console.log(`Batch mutation success (${variables.action} on ${table}). Invalidating keys:`, queriesToInvalidate)
