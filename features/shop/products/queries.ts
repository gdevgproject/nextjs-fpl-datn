...
export async function getFilterOptions(searchParams: { [key: string]: string | string[] | undefined }) {
...
export async function getProducts(searchParams: { [key: string]: string | string[] | undefined }, page = 1, limit = 12) {
    const supabase = await getSupabaseServerClient();
    ...
    try {
        ...
        if (searchParams.category) {
            ...
            const { data: brand, error: brandError } = await supabase.from("brands").select("id, name, description").eq("id", brandId).single();
        ...
        // Xây dựng select query cho truy vấn chính
        const selectQuery = `
      id,
      name,
      slug,
    ...
    `;
        ...
        // Lọc theo danh mục - Cách tiếp cận mới
        if (searchParams.category && categoryInfo) {
            // Trước tiên, lấy tất cả các product_id từ bảng product_categories
            ...
            if (pcError) {
                ...
                throw new Error(`Failed to fetch products by category: ${pcError.message}`);
            }
        ...
        if (searchParams.minPrice || searchParams.maxPrice || searchParams.sale === "true" || searchParams.featured === "true") {
            let variantQuery = supabase.from("product_variants").select("product_id, price, sale_price").is("deleted_at", null);
            ...
            if (searchParams.minPrice || searchParams.maxPrice) {
                ...
                const maxPrice = searchParams.maxPrice ? Number.parseInt(searchParams.maxPrice as string) : Number.MAX_SAFE_INTEGER;
                ...
                variants.forEach((variant: any) => {
                    const effectivePrice = variant.sale_price !== null && variant.sale_price > 0 ? variant.sale_price : variant.price;

                    // Nếu chưa có giá cho sản phẩm này hoặc giá mới thấp hơn giá đã lưu
                    if (!productMinPriceMap.has(variant.product_id) || effectivePrice < productMinPriceMap.get(variant.product_id)!) {
        ...
        if (searchParams.featured === "true") {
            // Lấy ids từ bảng product_label_assignments với nhãn "featured"
            const { data: featuredData, error: featuredError } = await supabase
                .from("product_label_assignments")
                .select("product_id")
                .eq("label_id", 1) // Giả sử ID 1 là nhãn "Featured"/"Nổi bật"
                .gte("valid_until", new Date().toISOString()); // Chỉ lấy nhãn còn hiệu lực

            if (featuredError) {
                console.error("Error fetching featured products:", featuredError);
            } else if (featuredData && featuredData.length > 0) {
                const featuredIds = featuredData.map((f) => f.product_id);
                // Nếu đã có productIdsFilter, lọc giao giữa 2 danh sách
                if (productIdsFilter) {
                    productIdsFilter = productIdsFilter.filter((id) => featuredIds.includes(id));
                } else {
                    productIdsFilter = featuredIds;
                }
   ...
   `
     ...
     `
            ...
            if (newError) {
                ...
                resultData = combinedProducts.filter((product: any, index: number, self: any[]) => index === self.findIndex((p) => p.id === product.id));
            }
        }
