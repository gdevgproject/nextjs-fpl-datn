  ...
  return (
    <div className="container py-8">
      <div className="flex flex-col gap-4">
        ...
        <p className="text-muted-foreground">Khám phá các thương hiệu nước hoa cao cấp tại MyBeauty</p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            <Suspense fallback={<BrandCardSkeletons count={5} />}>
              {brandsWithLogo.map((brand) => (
                <BrandCard key={brand.id} brand={brand} productCount={productCounts[brand.id] || 0} />
              ))}
            </Suspense>
          </div>
        </div>
