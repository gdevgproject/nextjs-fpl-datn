...
export function Header({ shopLogo, categories, genders, loadingCategories, loadingGenders, initialSettings, loadingSettings }: HeaderProps) {
    ...
    const isItemActive = useCallback(
        (href: string) => {
            if (pathname !== "/san-pham") return pathname === href;

            // Kiểm tra các tham số URL cho trang sản phẩm
            const urlParams = new URLSearchParams(href.split("?")[1] || "");
            const pageParams = new URLSearchParams(searchParams.toString());

            // Kiểm tra gender param
            if (urlParams.has("gender") && pageParams.has("gender")) {
                return urlParams.get("gender") === pageParams.get("gender");
            }

            // Kiểm tra category param
            if (urlParams.has("category") && pageParams.has("category")) {
                return urlParams.get("category") === pageParams.get("category");
            }
    ...
    const isSettingsLoading = initialSettings !== undefined ? loadingSettings : settingsHook.isLoading;
    const { data: session } = useAuthQuery();
    const isAuthenticated = !!session?.user;
    const logoUrl = shopLogo || settings?.shop_logo_url || "/placeholder-logo.svg";
    ...
    return (
        <header
            className={cn(
                ...
                scrolled ? "bg-background/90 backdrop-blur-lg shadow-md" : "bg-background/80 backdrop-blur-sm"
            )}
        >
            {/* Topbar */}
                    ...
                    <div className="flex items-center space-x-1.5 md:space-x-3">
                        ...
                        <div className="flex md:hidden items-center gap-1">
                            ...
                            <Sheet open={mobileMenuOpen} onOpenChange={handleMobileMenuToggle}>
                                ...
                                <SheetContent side="right" className="p-0 w-[85%] max-w-[320px]">
                                    <SheetTitle className="sr-only">{shopName ? `Menu - ${shopName}` : "Menu"}</SheetTitle>
                                    {/* Mobile nav content */}
            ...
            <nav
                className={cn(
                    ...
                    scrolled ? "border-transparent border-b-border/30" : "border-border/30"
                )}
            >
                            ...
                            <li key={item.href} className="flex-shrink-0">
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        isItemActive(item.href) ? "text-primary" : "text-muted-foreground hover:text-foreground"
                                    )}
                                >
                                    {item.name}
                                    {isItemActive(item.href) && <span className="absolute bottom-0 left-0 h-0.5 w-full bg-primary" />}
