  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (explicitPath) {
        ...
        // Create a structured path based on category context
        const categoryId = createPathOptions?.categoryId || "undefined";
        const uniqueId = uuidv4().substring(0, 8);
        const extension =
          createPathOptions?.fileExtension || fileExtension || "png";

        filePath = `categories/${categoryId}/${uniqueId}.${extension}`;
      }
    ...
    onSuccess: () => {
      // Invalidate categories list query to refresh data
      queryClient.invalidateQueries({ queryKey: ["categories", "list"] });
    },
    onError: (error) => {
      console.error(`Storage upload error (bucket: categories):`, error);
    },
  });
}
