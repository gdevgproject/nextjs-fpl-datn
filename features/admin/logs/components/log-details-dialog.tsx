...
export const LogDetailsDialog = memo(function LogDetailsDialog({
  log,
  logs = [],
  currentIndex = 0, // Mặc định là 0 (thay vì -1)
  open,
  onOpenChange,
  onNavigate,
  ...
  const changeDetails = useMemo(() => {
    if (!parsedDetails || log.activity_type.indexOf("UPDATE") === -1)
      return null;
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="changes" disabled={!changeDetails}>
              Thay đổi
            </TabsTrigger>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          `/admin/catalog/products/${log.entity_id}`,
                          "_blank"
                        )
                      }
                    >
          ...
          {logs.length > 1 && (
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() =>
                  onNavigate && canGoPrevious && onNavigate(currentIndex - 1)
                }
                className="h-8 w-8"
              >
              ...
              <Button
                ...
                disabled={!canGoNext}
                onClick={() =>
                  onNavigate && canGoNext && onNavigate(currentIndex + 1)
                }
                className="h-8 w-8"
              >
...
function DisplayValue({ value }: { value: any }) {
  if (value === null || value === undefined) {
    return (
      <span className="text-muted-foreground italic">Không có giá trị</span>
    );
  }
