...
export async function createProductVariantAction(
  ...
  try {
    ...
    const payload = {
      ...
      sale_price:
        validatedData.sale_price && validatedData.sale_price !== ""
...
export async function updateProductVariantAction(
  ...
  try {
    ...
    if (
      currentVariant &&
      currentVariant.stock_quantity !== payload.stock_quantity
    ) {
      const quantityChange =
        payload.stock_quantity - currentVariant.stock_quantity;
...
export async function updateProductIngredientsAction(
  ...
  try {
    ...
    if (deleteError) {
      console.error(
        "Error deleting existing product ingredients:",
        deleteError
      );
      return { error: deleteError.message, success: false };
    }
  ...
  try {
    ...
    try {
      ...
      if (path) {
        ...
        if (storageError) {
          console.error(
            "Error deleting product image from storage:",
            storageError
          );
