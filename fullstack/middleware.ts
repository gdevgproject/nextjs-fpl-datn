...
export async function middleware(req: NextRequest) {
  ...
  if (sessionError) {
    ...
    if (
      ...
      redirectUrl.searchParams.set("error_description", "Phiên đăng nhập không hợp lệ hoặc đã hết hạn")
  ...
  if (isAdminProtected && isAuthenticated) {
    try {
      ...
      if (
        roleError &&
        (roleError.message.includes("user does not exist") || roleError.message.includes("JWT subject not found"))
      ) {
        // Đăng xuất người dùng và chuyển hướng đến trang đăng nhập
        await supabase.auth.signOut()
        const redirectUrl = new URL("/dang-nhap", req.url)
        redirectUrl.searchParams.set("error", "account_deleted")
        redirectUrl.searchParams.set("error_description", "Tài khoản của bạn không còn tồn tại hoặc đã bị vô hiệu hóa")
        return NextResponse.redirect(redirectUrl)
      }
