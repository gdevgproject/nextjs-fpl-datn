...
export async function getProducts(
  ...
  try {
    ...
    if (searchParams.category) {
      ...
      const brandId = parseInt(searchParams.brand as string, 10);
    ...
    if (searchParams.brand) {
      query = query.eq("brand_id", parseInt(searchParams.brand as string, 10));
    }
    ...
    if (searchParams.gender) {
      query = query.eq(
        "gender_id",
        parseInt(searchParams.gender as string, 10)
      );
    }
    ...
    if (searchParams.perfume_type) {
      query = query.eq(
        "perfume_type_id",
        parseInt(searchParams.perfume_type as string, 10)
      );
    }
    ...
    if (searchParams.concentration) {
      query = query.eq(
        "concentration_id",
        parseInt(searchParams.concentration as string, 10)
      );
    }
    ...
    if (
      ...
      if (variantsError) {
        console.error(
          "Error fetching variants for price filtering:",
          variantsError
        );
      ...
      if (searchParams.minPrice || searchParams.maxPrice) {
        const minPrice = searchParams.minPrice
          ? Number.parseInt(searchParams.minPrice as string)
          : 0;
    ...
    if (priceSort) {
      products = products.sort((a: any, b: any) => {
        ...
        const priceA =
          a.sale_price !== null && a.sale_price > 0 ? a.sale_price : a.price;
        const priceB =
          b.sale_price !== null && b.sale_price > 0 ? b.sale_price : b.price;
