          ...
          <div className="flex-1">
            <div className="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
              <p className="text-sm text-muted-foreground">
                {count > 0 ? (
                  <>
                    Hiển thị {products.length > 0 ? (page - 1) * limit + 1 : 0}-
                    {Math.min(page * limit, count)} trong số {count} sản phẩm
                  </>
                ) : (
              ...
              <SortOptions
                currentSort={searchParams.sort as string | undefined}
              />
            </div>
    ...
    return (
      <div className="container py-8">
        <Alert variant="destructive" className="mb-6">
          ...
          <AlertDescription>
            Đã xảy ra lỗi khi tải dữ liệu sản phẩm. Vui lòng thử lại sau hoặc
        ...
        <div className="rounded-lg border p-6 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Có thể hệ thống đang bận. Vui lòng trở lại sau.
          </p>
        </div>
      </div>
    );
...
function getPageTitleAndFilters(
  ...
  let description =
    "Khám phá bộ sưu tập nước hoa chính hãng đa dạng của chúng tôi";
  ...
  if (categoryInfo) {
    ...
    description =
      ...
      `Khám phá các sản phẩm trong danh mục ${categoryInfo.name}`;
  ...
  if (brandInfo) {
    ...
    description =
      ...
      `Khám phá các sản phẩm từ thương hiệu ${brandInfo.name}`;
  ...
  if (searchParams.gender) {
    const gender = filterOptions.genders.find(
      (g: any) => g.id.toString() === (searchParams.gender as string)
    );
    if (gender) {
  ...
  if (searchParams.minPrice || searchParams.maxPrice) {
    ...
    const matchedRange = filterOptions.priceRanges.find(
      (r: any) => r.min === min && r.max === max
    );
