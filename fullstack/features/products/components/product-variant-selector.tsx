  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            {selectedVariant && (
              <p className="text-sm text-muted-foreground">
                {isOutOfStock ? (
                  <span className="text-red-500">Hết hàng - Vui lòng chọn sản phẩm khác</span>
                ) : (
                  <>
                    Còn lại: <span className="font-medium">{selectedVariant.stock_quantity}</span> sản phẩm
                  </>
                )}
              </p>
            )}
          </div>
          <RadioGroup value={selectedVariantId?.toString()} onValueChange={handleVariantChange}>
              ...
              {variants.map((variant: any) => (
                <div key={variant.id} className="flex items-center space-x-2">
                  ...
                  >
                    ...
                    {variant.sale_price ? (
                      <span className="ml-2 text-primary">{formatPrice(variant.sale_price)}</span>
                    ) : (
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        <Button
          variant="outline"
          className={`gap-2 ${isWishlisted ? "border-primary text-primary hover:bg-primary/10" : ""}`}
