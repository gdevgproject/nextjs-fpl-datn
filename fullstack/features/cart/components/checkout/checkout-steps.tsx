...
import { CheckCircle, CreditCard, FileText, MapPin, ChevronRight } from "lucide-react"
    ...
    <div className="border rounded-lg p-4 bg-card">
      <div className="flex items-center justify-between">
        {steps.map((step, index) => {
          ...
          const isCompleted = getStepStatus(currentStep, step.id) === "completed"
          ...
          return (
            <div key={step.id} className="flex items-center">
              ...
              {index < steps.length - 1 && <ChevronRight className="h-4 w-4 mx-1 sm:hidden text-muted-foreground" />}
            </div>
          )
        })}
      </div>
...
function getStepStatus(currentStep: CheckoutStep, stepId: CheckoutStep): "pending" | "current" | "completed" {
