...
export function ProductBulkActions({
  ...
  const handleBulkRestore = async () => {
    try {
      ...
      if (result.success) {
        toast.success(
          `Đã khôi phục ${result.affectedCount} sản phẩm thành công`
        );
        onActionComplete();
      } else {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Đã thêm danh mục cho ${result.affectedCount} sản phẩm thành công`
        );
        onActionComplete();
      } else {
  ...
  const handleRemoveCategory = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Đã xóa danh mục khỏi ${result.affectedCount} sản phẩm thành công`
        );
        onActionComplete();
      } else {
  ...
  const handleAddLabel = async () => {
    ...
    try {
      const result = await bulkAddLabel.mutateAsync({
        ...
        valid_until: labelValidUntil
          ? labelValidUntil.toISOString()
          : undefined,
      });

      if (result.success) {
        toast.success(
          `Đã thêm nhãn cho ${result.affectedCount} sản phẩm thành công`
        );
        onActionComplete();
      } else {
  ...
  const handleRemoveLabel = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Đã xóa nhãn khỏi ${result.affectedCount} sản phẩm thành công`
        );
        onActionComplete();
      } else {
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center p-2 bg-muted/50 rounded-md">
      ...
      >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn xóa {selectedProductIds.length} sản phẩm đã
              chọn không? Hành động này sẽ chỉ ẩn sản phẩm khỏi cửa hàng và có
              thể khôi phục sau này.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn khôi phục {selectedProductIds.length} sản
              phẩm đã chọn không? Hành động này sẽ hiển thị lại sản phẩm trong
              cửa hàng.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {/* Add Category Dialog */}
      <Dialog
        open={activeDialog === "add-category"}
        onOpenChange={(open) => !open && setActiveDialog(null)}
      >
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setActiveDialog("add-category")}
          >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Chọn danh mục để thêm vào {selectedProductIds.length} sản phẩm đã
              chọn.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        open={activeDialog === "remove-category"}
        onOpenChange={(open) => !open && setActiveDialog(null)}
      >
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setActiveDialog("remove-category")}
          >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Chọn danh mục để xóa khỏi {selectedProductIds.length} sản phẩm đã
              chọn.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setActiveDialog("add-label")}
          >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Chọn nhãn để thêm vào {selectedProductIds.length} sản phẩm đã chọn
              và tùy chọn thời hạn hiệu lực.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!selectedLabelId || bulkAddLabel.isPending}
            >
      ...
      {/* Remove Label Dialog */}
      <Dialog
        open={activeDialog === "remove-label"}
        onOpenChange={(open) => !open && setActiveDialog(null)}
      >
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setActiveDialog("remove-label")}
          >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Chọn nhãn để xóa khỏi {selectedProductIds.length} sản phẩm đã
              chọn.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!selectedLabelId || bulkRemoveLabel.isPending}
            >
