    ...
    return (
      <div className="flex flex-wrap gap-2 mb-4 mt-2">
        {filter.brand_id && lookups?.brands && (
          <FilterBadge
            label={`Thương hiệu: ${
              lookups.brands.find((b) => b.id === filter.brand_id)?.name || ""
            }`}
        ...
        {filter.gender_id && lookups?.genders && (
          <FilterBadge
            label={`Giới tính: ${
              lookups.genders.find((g) => g.id === filter.gender_id)?.name || ""
            }`}
        ...
        {filter.perfume_type_id && lookups?.perfumeTypes && (
          <FilterBadge
            label={`Loại: ${
              lookups.perfumeTypes.find(
                (t) => t.id === filter.perfume_type_id
              )?.name || ""
            }`}
        ...
        {filter.concentration_id && lookups?.concentrations && (
          <FilterBadge
            label={`Nồng độ: ${
              lookups.concentrations.find(
                (c) => c.id === filter.concentration_id
              )?.name || ""
            }`}
        ...
        {filter.in_stock && (
          <FilterBadge
            label="Còn hàng"
            onRemove={() => onFilterChange({ in_stock: undefined })}
          />
        )}
        {filter.has_promotion && (
          <FilterBadge
            label="Có khuyến mãi"
            onRemove={() => onFilterChange({ has_promotion: undefined })}
          />
        )}
        {filter.deleted && (
          <FilterBadge
            label="Đã ẩn"
            onRemove={() => onFilterChange({ deleted: undefined })}
          />
        )}
        {filter.categories &&
        ...
        {filter.labels && filter.labels.length > 0 && lookups?.productLabels && (
          <FilterBadge
            label={`Nhãn: ${filter.labels.length}`}
            onRemove={() => onFilterChange({ labels: undefined })}
          />
        )}
        <Button
          variant="ghost"
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleBooleanFilterChange("in_stock", !!checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleBooleanFilterChange("has_promotion", !!checked)
                  }
                />
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleBooleanFilterChange("deleted", !!checked)
                  }
                />
                  ...
                  {lookups?.categories.map((category) => (
                    ...
                    >
                      <Checkbox
                        ...
                        checked={
                          filter.categories?.includes(category.id) || false
                        }
                        onCheckedChange={(checked) =>
