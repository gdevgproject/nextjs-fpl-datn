...
export async function getProducts(
  ...
  try {
    ...
    if (validParams.min_price !== null && validParams.min_price !== undefined) {
      query = query.in(
        ...
        supabase
          .from("product_variants")
          .select("product_id")
          .gte("price", validParams.min_price)
      );
    }
    ...
    if (validParams.max_price !== null && validParams.max_price !== undefined) {
      query = query.in(
        ...
        supabase
          .from("product_variants")
          .select("product_id")
          .lte("price", validParams.max_price)
      );
    }
    ...
    const formattedData = data.map((product) => {
      ...
      const hasPromotion =
        product.variants.some((v) => v.sale_price !== null) || false;
    ...
    if (validParams.sort_by === "price") {
      formattedData.sort((a, b) => {
        const aPrice = Math.min(
          ...a.variants.map((v) => v.sale_price ?? v.price)
        );
        const bPrice = Math.min(
          ...b.variants.map((v) => v.sale_price ?? v.price)
        );
        return validParams.sort_order === "asc"
          ? aPrice - bPrice
          : bPrice - aPrice;
      });
    }
...
export async function updateProduct(
  ...
  try {
    ...
    const updatedVariantIds = validatedData.variants
      .filter((v) => v.id)
    ...
    if (deleteCategoriesError) {
      throw new Error(
        `Failed to clear existing categories: ${deleteCategoriesError.message}`
      );
    }
    ...
    if (deleteLabelsError) {
      throw new Error(
        `Failed to clear existing labels: ${deleteLabelsError.message}`
      );
    }
  ...
  try {
    ...
    if (updates.is_main) {
      ...
      if (updateMainError) {
        ...
        throw new Error(
          `Failed to update main image status: ${updateMainError.message}`
        );
      }
    }
...
export async function bulkAssignCategoryToProducts(
  params: BulkCategoryActionParams
): Promise<BulkActionResponse> {
...
export async function bulkRemoveCategoryFromProducts(
  params: BulkCategoryActionParams
): Promise<BulkActionResponse> {
  const { productIds, categoryId } = params;
...
export async function bulkAssignLabelToProducts(
  params: BulkLabelActionParams
): Promise<BulkActionResponse> {
...
export async function bulkRemoveLabelFromProducts(
  params: BulkLabelActionParams
): Promise<BulkActionResponse> {
