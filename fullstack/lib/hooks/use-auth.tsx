...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkUserRoles = async () => {
    try {
      ...
      if (
        isAdminError &&
        (isAdminError.message.includes("user does not exist") ||
          isAdminError.message.includes("JWT subject not found"))
      ) {
        await handleDeletedAccount();
        return;
      }
      ...
      const { data: isStaffData, error: isStaffError } = await supabase.rpc(
        "is_staff"
      );
      ...
      if (
        isStaffError &&
        (isStaffError.message.includes("user does not exist") ||
          isStaffError.message.includes("JWT subject not found"))
      ) {
        await handleDeletedAccount();
        return;
      }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Đã xảy ra lỗi khi gửi liên kết đăng nhập",
      };
    } finally {
      setIsLoading(false);
    }
      ...
      if (error) {
        ...
        if (
          ...
          return {
            success: false,
            error:
              "Đã vượt quá giới hạn gửi email. Vui lòng thử lại sau 30 phút.",
