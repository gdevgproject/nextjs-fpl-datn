...
export function getErrorFromURL(url: URL): { hasError: boolean; errorCode?: string; errorMessage?: string } {
  const errorCode = url.searchParams.get("error_code") || url.hash.match(/error_code=([^&]*)/)?.[1]
  const errorMessage = url.searchParams.get("error_description") || url.hash.match(/error_description=([^&]*)/)?.[1]

  if (errorCode || errorMessage) {
    return {
      hasError: true,
      errorCode: errorCode || undefined,
      errorMessage: errorMessage ? decodeURIComponent(errorMessage.replace(/\+/g, " ")) : undefined,
    }
  }
